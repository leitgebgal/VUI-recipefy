<template>
    <v-container>
      <v-card :class="(recipe.flag) ? 'recipe-card flagged' : 'recipe-card'">
          ...
          <v-col class="pa-4">
            <v-row class="align-center justify-center">
              <v-card-title class="recipe-title">
                {{ recipe.title }}
                <v-icon v-if="recipe.flag" icon="mdi-flag" color="red"></v-icon>
              </v-card-title>
            </v-row>
            <v-row class="align-center justify-center">
              <v-img :src="recipe.imageUrl" cover max-width="600px" height="350px" :alt="recipe.title" class="recipe-image mb-6" />
            </v-row>
            <v-chip v-for="cat in recipe.categories" :key="cat" :color="getCategoryColor(cat)" :prepend-icon="getCategoryIcon(cat)" class="category-chip">
              {{ cat }}
            </v-chip>
            ...
            <div class="ingredients-section">
              ...
              <v-expand-transition>
                <ul v-show="ingredientsVisible" class="recipe-list">
                  <li v-for="ingredient, index in recipe.ingredients" :key="ingredient" class="recipe-list-item">
                    {{ displayIngredient(ingredient, index) }}
                  </li>
                </ul>
              </v-expand-transition>
              <v-btn @click="toggleIngredients">{{ ingredientsVisible ? 'Hide Ingredients' : 'Show Ingredients' }}</v-btn>
            </div>
        ...
        <div v-if="user != null && user.email != recipe.createdBy" class="d-flex flex-column">
          <div class="ma-2 pa-2">
            <span v-for="star in stars" :key="star" @mouseover="hoverRating(star)" @mouseleave="resetHover" @click="selectRating(star)">
              <v-icon :color="star <= selectedRating ? 'amber' : ''" v-bind:class="{ 'hovered': star <= hoveredRating }">
  ...
  const ingredientsVisible = ref(false);
  ...
  console.log("hotjar event: comment_added B");
  hj('event', 'comment_added_B', {
  ...
  const addRating = () => {
  // Pošiljanje dogodka v Hotjar
  console.log("hotjar event: recipe_rating_submitted B");
  hj('event', 'recipe_rating_submitted_B', {
    recipeTitle: recipe.value.title,
